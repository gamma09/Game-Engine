...
void stbi_decode_DXT1_block( unsigned char uncompressed[16*4], unsigned char compressed[8] )
{
	...
	decode_colors[0] = (uint8) r;
	decode_colors[1] = (uint8) g;
	decode_colors[2] = (uint8) b;
	decode_colors[3] = 255;
	stbi_rgb_888_from_565( c1, &r, &g, &b );
	decode_colors[4] = (uint8) r;
	decode_colors[5] = (uint8) g;
	decode_colors[6] = (uint8) b;
	decode_colors[7] = 255;
	if( c0 > c1 )
	{
			...
			unsigned char compressed[8] )
{
	...
	for( i = 3; i < 16*4; i += 4 )
	{
		uncompressed[i] = (uint8) stbi_convert_bit_range(
			...
			unsigned char compressed[8] )
{
	...
	decode_colors[2] = (uint8) b;
	stbi_rgb_888_from_565( c1, &r, &g, &b );
	decode_colors[3] = (uint8) r;
	decode_colors[4] = (uint8) g;
	decode_colors[5] = (uint8) b;
